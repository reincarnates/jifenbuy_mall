    ...
    uploadDetailImage: function (e) { //这里是选取图片的方法
      ...
      wx.chooseImage({
        ...
        success: function (res) {
          ...
          that.triggerEvent('myevent2', myEventDetail, myEventOption)//结果返回调用的页面
          // that.uploadimg({
    ...
    uploadimg: function (data) {
      ...
      wx.uploadFile({
        ...
        success: (resp) => {
          ...
          var pic_name = that.data.showUrl + pic.imgUrl;
          var detailPics = that.data.detailPics2;
          detailPics.push(pic_name)
          that.setData({
            detailPics2: detailPics
          })
        },
        fail: (res) => {
          fail++;
        ...
        complete: () => {
          ...
          if (i == data.path.length) { //当图片传完时，停止调用     
            ...
            var myEventDetail = {
              picsList: that.data.detailPics2
            } // detail对象，提供给事件监听函数
            var myEventOption = {} // 触发事件的选项
